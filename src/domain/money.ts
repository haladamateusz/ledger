...
export function parseDecimalToMinorUnits(amount: string, currency: Currency): number {
    ...
    if (fractionalPart.length > exponent) {
        throw new Error(`Too many decimal places for ${currency}: expected at most ${exponent}`);
    }
