...
describe("parseDecimalToMinorUnits", () => {
    ...
    it("rejects too many decimal places", () => {
        expect(() => parseDecimalToMinorUnits("12.345", "EUR")).toThrow(
            "Too many decimal places",
        );
    });

    it("rejects invalid decimal strings", () => {
        expect(() => parseDecimalToMinorUnits("", "EUR")).toThrow(
            "Invalid decimal amount",
        );
        expect(() => parseDecimalToMinorUnits("12.", "EUR")).toThrow(
            "Invalid decimal amount",
        );
        expect(() => parseDecimalToMinorUnits(".12", "EUR")).toThrow(
            "Invalid decimal amount",
        );
        expect(() => parseDecimalToMinorUnits("12,34", "EUR")).toThrow(
            "Invalid decimal amount",
        );
    });
});